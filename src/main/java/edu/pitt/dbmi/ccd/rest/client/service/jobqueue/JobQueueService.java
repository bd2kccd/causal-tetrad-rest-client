...
public class JobQueueService extends AbstractRequestService implements AbstractJobQueueRequest {
    ...
    public JobQueueService(RestHttpsClient restHttpsClient,
	    String scheme, String hostname, int port) {
	super(restHttpsClient, scheme, hostname, port);
    }

    public JobInfo addToRemoteQueue(String AlgorithmName, AlgorithmParamRequest algoParamRequest, JsonWebToken jsonWebToken) throws URISyntaxException,
    ...
    public List<JobInfo> getActiveJobs(JsonWebToken jsonWebToken) throws URISyntaxException,
    ClientProtocolException, IOException {
	URIBuilder uriBuilder = new URIBuilder().setHost(hostname)
		.setScheme(scheme)
		.setPath("/" + REST_API + "/" + jsonWebToken.getUserId() + "/" + JOBS)
		.setPort(port);
    ...
    public JobInfo getJobStatus(long id, JsonWebToken jsonWebToken) throws URISyntaxException,
    ClientProtocolException, IOException {
	URIBuilder uriBuilder = new URIBuilder().setHost(hostname)
		.setScheme(scheme)
		.setPath("/" + REST_API + "/" + jsonWebToken.getUserId() + "/" + JOBS + "/" + id)
		.setPort(port);
